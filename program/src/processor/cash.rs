...
pub fn process_redemption(
    ...
    let mut fee_to_redeem = cash.max_fee_to_redeem()?;
    ...
    if cash.is_fully_redeemed()? {
        if is_native
        {
            ...
            if remaining > 0 {
                native_transfer(fee_payer_info, owner_wallet_info, remaining, &[])?;
            }
        } else {
            if remaining > 0 {
                spl_token_transfer(
